...
xn = [0.6,-0.9]; %punto iniziale
xc = 0.7; %centro del target
yc = 0;
lambda = 1; %discount factor
gamma = 1; %peso del controllo

h = 0.05;
dx = 0.05;
epsi = 1e-6;
f = @(x,y,a) [ y/(sqrt(x^2+y^2) + epsi), -x/(sqrt(x^2+y^2) + epsi) ]  + a;
rho = linspace(0,1,4);
rho = rho(2:end);
...
while (xn(1) - xc)^2 + (xn(2) - yc)^2 > radius^2 && it1 < it1max
    ...
    arg = 1e3*ones(Na,1);
    p(1).XData = [p(1).XData xn(1)];
    p(1).YData = [p(1).YData xn(2)];
    drawnow;
    pause(0.05);
    it1 = it1+1; 
end
